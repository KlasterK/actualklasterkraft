...
#include "prebuiltconfiguration.hpp"
#include "session.hpp"
...
asio::awaitable<sys::error_code> statecoroutines::configuration(
    ...
    std::array<uint8_t, 16> &&player_uuid)
{
    ...
    session->get_streambuf().sputn(
        reinterpret_cast<const char *>(PrebuiltClientboundKnownPackets.data()),
